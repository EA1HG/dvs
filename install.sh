...
if dpkg -l dvswitch-monit | grep "Monitor DVSwitch services"; then
    # Desinstalar dvswitch-server
    sudo apt-get remove dvswitch-server -y

    # Eliminar dvswitch-monit
    sudo dpkg --purge dvswitch-monit
...
chmod +x /bin/DVS
#
sudo cat > /var/lib/dvswitch/dvs/var.txt <<- "EOFV"

source /var/lib/dvswitch/dvs/func.txt
source /var/lib/dvswitch/dvs/lan/language.txt
...
wget --load-cookies /tmp/cookies.txt "https://docs.google.com/uc?export=download&confirm=$(wget --quiet --save-cookies /tmp/cookies.txt --keep-session-cookies --no-check-certificate 'https://docs.google.com/uc?export=download&id=1y9xpzmPIEiGyp0kLZYZaT1vMS20Xb48z' -O- | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\1\n/p')&id=1y9xpzmPIEiGyp0kLZYZaT1vMS20Xb48z" -O /var/www/html/DVS-Dash.zip && rm -rf /tmp/cookies.txt &&
cd /var/www/html
...
EOF
# 
sudo systemctl restart quantar_bridge.service
fi
...
sudo sed -i "s/FileLevel=.*/FileLevel=1/g"  /opt/MMDVM_Bridge/MMDVM_Bridge.ini
##########
if systemctl status apache2.service |grep active >/dev/null 2>&1
then 
...
###########
if systemctl status analog_bridge.service |grep disabled >/dev/null 2>&1
then 
 sudo systemctl enable analog_bridge.service
fi
if systemctl status mmdvm_bridge.service |grep disabled >/dev/null 2>&1
then 
 sudo systemctl enable mmdvm_bridge.service
fi
if systemctl status ysfgateway.service |grep disabled >/dev/null 2>&1
then 
 sudo systemctl enable ysfgateway.service
fi
if systemctl status ysfparrot.service |grep disabled >/dev/null 2>&1
then 
 sudo systemctl enable ysfparrot.service
fi
if systemctl status nxdngateway.service |grep  disabled >/dev/null 2>&1
then 
 sudo systemctl enable nxdngateway.service
fi
if systemctl status nxdnparrot.service |grep disabled >/dev/null 2>&1
then 
 sudo systemctl enable nxdnparrot.service
fi
if systemctl status p25gateway.service |grep disabled >/dev/null 2>&1
then 
 sudo systemctl enable p25gateway.service
fi
if systemctl status p25parrot.service |grep disabled >/dev/null 2>&1
then 
 sudo systemctl enable p25parrot.service
fi
if systemctl status quantar_bridge.service |grep disabled >/dev/null 2>&1
then 
 sudo systemctl enable quantar_bridge.service
fi
if systemctl status ircddbgatewayd.service |grep disabled >/dev/null 2>&1
then 
 sudo systemctl enable ircddbgatewayd.service
fi
if systemctl status md380-emu.service |grep disabled >/dev/null 2>&1
then 
 sudo systemctl enable md380-emu.service
fi
if systemctl status apache2.service |grep disabled >/dev/null 2>&1
then 
